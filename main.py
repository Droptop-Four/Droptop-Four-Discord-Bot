...
bot.newversionping = int(os.getenv("newversionping")
                         )   # Newversionping Role ID
...
bot.dsannchannel = int(os.getenv("dsannchannel"))
bot.infodeskchannel = int(os.getenv("infodeskchannel"))  # Infodesk Channel ID
bot.botcommandschannel = int(
    os.getenv("botcommandschannel"))  # BotCommand Channel ID
...
async def _rero_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        ...
        embed.add_field(name='{}reactrole command'.format(bot.command_prefix),
                        value='{}reactrole <emoji> <@role> <message>'.format(bot.command_prefix), inline=False)
...
async def bothelp(ctx):
    ...
    adminrole = discord.utils.get(
        ctx.guild.roles, id=bot.adminrole_id)  # Admin Role ID
    modrole = discord.utils.get(
        ctx.guild.roles, id=bot.modrole_id)  # Moderator Role ID

    staffemb = discord.Embed(title='Bot Commands',
                             color=discord.Color.from_rgb(75, 215, 100))
    staffemb.set_author(
        name="Droptop Four", url="https://blacksquare88.wixsite.com/droptop4", icon_url=bot.user.avatar_url)
    staffemb.set_thumbnail(url=ctx.guild.icon_url)
    staffemb.add_field(name="Developer", value='`{0}listcogs`\nReturns a list of all the enabld cogs.\n`{0}load`\nLoads a cog.\n`{0}reload`\nReloads a cog.\n`{0}unload`\nUnloads a cog.'.format(
        bot.command_prefix), inline=False)
    staffemb.add_field(
        name="Reactionrole", value='`{0}reactrole <emoji> <@role> <message>`\nUse it to create a new reactionrole in your current channel.'.format(bot.command_prefix), inline=False)
    staffemb.add_field(name="Announcements", value='`{3}brdt`\nCreates a Droptop Announcement in <#{0}>.\n`{3}brds`\nCreates a Discord Announcement in <#{1}>.\n`{3}brnv`\nCreates a New Version Announcement <#{2}>.'.format(
        (bot.annchannel), (bot.dsannchannel), (bot.annchannel), (bot.command_prefix)), inline=False)
    staffemb.add_field(name="Polls", value='`{0}poll <emoji1> <emoji2>`\nCreates a poll with the 2 emojis as voting reactions in your current channel.'.format(
        bot.command_prefix), inline=False)
    ...
    memberemb = discord.Embed(title='Bot Commands',
                              color=discord.Color.from_rgb(75, 215, 100))
    memberemb.set_author(
        name="Droptop Four", url="https://blacksquare88.wixsite.com/droptop4", icon_url=bot.user.avatar_url)
    ...
    memberemb.add_field(name="Suggestions", value='`{0}suggest`\nSends a suggestion to <#{1}>.'.format(
        (bot.command_prefix), (bot.suggchannel)), inline=False)
