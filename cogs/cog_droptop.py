...
class DroptopCommands(commands.Cog):
    ...
    async def community_app_new_release(
        ...
        if rmskin_name_check("app", rmskin_package.filename):
            ...
            await interaction.response.send_message(
                "No rmskin app package was found, be sure to put it in the right hitbox the next time.",
                ephemeral=True,
            )
    ...
    async def community_theme_new_release(
        self,
        interaction: discord.Interaction,
        rmskin_package: discord.Attachment,
        image_preview: discord.Attachment,
    ):
        ...
        if rmskin_name_check("theme", rmskin_package.filename):
            ...
            if available:
                ...
                await interaction.response.send_message(
                    "The name you chose is not available, choose another one for your theme",
                    ephemeral=True,
                )
        else:
            await interaction.response.send_message(
                "No rmskin theme package was found, be sure to put it in the right hitbox the next time.",
                ephemeral=True,
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if interaction.user.id in authorised_members:
            if self.image_mode == "jpg":
                ...
                rmskin_creation = push_rmskin(
                    self.configs["github_private_key"], "theme", rmskin_name
                )
                ...
                if self.description.value:
                    embed = discord.Embed(
                        title=f"{self.theme_title} - {self.author}",
                        description=f"{self.description.value}",
                        color=discord.Color.from_rgb(75, 215, 100),
                    )
                else:
                ...
                for thread in threads:
                    ...
                    await self.channel.create_thread(
                        name=f"{self.theme_title} - {self.author}",
                        embed=embed,
                        file=image_file,
                        view=view,
                    )
                webp_path.unlink()
                await interaction.followup.send(
                    f"You successfully published **{self.theme_title}** in <#{self.channel.id}>",
                    ephemeral=True,
                )
            else:
                await interaction.response.send_message(
            ...
            await interaction.followup.send(
                f"You aren't authorised to publish updates, modify or delete {self.theme_title}.\nAsk {self.author} to add you as an authorised user.",
                ephemeral=True,
            )
