...
class DevCommands(commands.Cog):
    ...
    @app_commands.describe(cog="The cog you want to reload")
    ...
    async def reload_cog(self, interaction: discord.Interaction, cog: str):
        ...
        if cog not in cogs_list:
            return await interaction.response.send_message(
                f"The `{cog}` cog doesn't exist.", ephemeral=True
            )
        else:
            ...
            embed = discord.Embed(
                title="Reloaded Cog",
                description=f"{interaction.user.mention} reloaded the `{cog}` cog",
    ...
    async def unload_cog(self, interaction: discord.Interaction, cog: str):
        ...
        if cog not in cogs_list:
            ...
            await interaction.followup.send(
                f"`{cog}` successfully unloaded.", ephemeral=True
            )
            log_channel = interaction.guild.get_channel(
                self.bot.configs["moderationlog_channel"]
            )
            embed = discord.Embed(
                title="Unloaded Cog",
                description=f"{interaction.user.mention} unloaded the `{cog}` cog",
                color=discord.Colour.from_rgb(213, 39, 62),
            )
            await log_channel.send(embed=embed)
    ...
    async def load_cog(self, interaction: discord.Interaction, cog: str):
        ...
        if cog not in cogs_list:
            ...
            await self.bot.tree.sync()
            await interaction.followup.send(
                f"`{cog}` successfully loaded.", ephemeral=True
            )
            log_channel = interaction.guild.get_channel(
                self.bot.configs["moderationlog_channel"]
            )
            embed = discord.Embed(
                title="Loaded Cog",
                description=f"{interaction.user.mention} loaded the `{cog}` cog",
                color=discord.Colour.from_rgb(213, 39, 62),
            )
            await log_channel.send(embed=embed)
    ...
    async def sync_commands(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "Syncing command tree...", ephemeral=True
        )
        self.bot.tree.clear_commands(
            guild=self.bot.get_guild(self.bot.configs["server_id"])
        )
        ...
        embed = discord.Embed(
            ...
            description=f"{interaction.user.mention} synced the commands",
            color=discord.Colour.from_rgb(213, 39, 62),
        )
        await log_channel.send(embed=embed)
