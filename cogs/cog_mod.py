...
class ModerationCommands(commands.Cog):
	...
	async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = None):
		...
		log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
		embed = discord.Embed(title="Member kicked", description=f"{interaction.user.mention} kicked {member.mention}", color=discord.Colour.from_rgb(213, 39, 62))
	...
	async def ban(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = None):
		...
		log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
		embed = discord.Embed(title="Member banned", description=f"{interaction.user.mention} banned {member.mention}", color=discord.Colour.from_rgb(213, 39, 62))
		embed.add_field(name="Reason", value=reason)
		await log_channel.send(embed=embed)
	...
	async def unban(self, interaction: discord.Interaction, member: str, reason: Optional[str] = None):
		...
		async for entry in interaction.guild.bans():
			if member.lower() == entry.user.name.lower():
				...
				log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
				embed = discord.Embed(title="Member unbanned", description=f"{interaction.user.mention} unbanned {banned_user.mention}", color=discord.Colour.from_rgb(213, 39, 62))
				embed.add_field(name="Reason", value=reason)
				await log_channel.send(embed=embed)
	...
	async def purge(self, interaction: discord.Interaction, amount: Optional[int] = 15, reason: Optional[str] = None):
		...
		log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
		embed = discord.Embed(title="Channel Purged", description=f"{interaction.user.mention} purged {interaction.channel.mention}", color=discord.Colour.from_rgb(213, 39, 62))
	...
	async def timeout(self, interaction: discord.Interaction, member: discord.Member, duration: Optional[str] = "60 secs", reason: Optional[str] = None):
		...
		if duration in ["60 secs", "5 mins", "10 mins", "1 hour", "1 day", "1 week"]:
			...
			log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
			embed = discord.Embed(title="Member timed out", description=f"{interaction.user.mention} timed out {member.mention}", color=discord.Colour.from_rgb(213, 39, 62))
	...
	async def timeout_remove(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = None):
		...
		if member.is_timed_out():
			...
			log_channel = interaction.guild.get_channel(self.bot.configs["moderationlog_channel"])
			embed = discord.Embed(title="Removed time out", description=f"{interaction.user.mention} removed the time out from {member.mention}", color=discord.Colour.from_rgb(213, 39, 62))
			embed.add_field(name="Reason", value=reason)
			await log_channel.send(embed=embed)
