...
class ModerationCommands(commands.Cog):
    ...
    async def report_message(
        ...
        embed = discord.Embed(
            title="Reported Message", color=discord.Colour.from_rgb(213, 39, 62)
        )
        if message.content:
    ...
    async def report_member(
        ...
        await interaction.response.send_message(
            f"Thanks for reporting {member.mention} to our moderators.", ephemeral=True
        )
        log_channel = interaction.guild.get_channel(
            self.bot.configs["moderationlog_channel"]
        )
        embed = discord.Embed(
            title="Reported Member", color=discord.Colour.from_rgb(213, 39, 62)
        )
        embed.set_author(name=member.display_name, icon_url=member.display_avatar.url)
        embed.description = (
            f"{member.mention} was reported by {interaction.user.mention}."
        )
        await log_channel.send(embed=embed)

    @app_commands.command(name="kick")
    @app_commands.describe(
        member="The member you want to kick", reason="The reason for the kick"
    )
    ...
    async def kick(
        ...
        embed = discord.Embed(
            ...
            color=discord.Colour.from_rgb(213, 39, 62),
        )
        embed.add_field(name="Reason", value=reason)
        await log_channel.send(embed=embed)
        await interaction.response.send_message(
            f"{member.mention} successfully kicked.", ephemeral=True
        )

    @app_commands.command(name="ban")
    @app_commands.describe(
        member="The member you want to ban", reason="The reason for the ban"
    )
    @app_commands.guild_only()
    @app_commands.default_permissions(manage_channels=True)
    async def ban(
        ...
        await log_channel.send(embed=embed)
        await interaction.response.send_message(
            f"{member.mention} successfully banned.", ephemeral=True
        )

    async def banned_members_autocomplete(
        self,
        interaction: discord.Interaction,
        current: str,
    ...
    @app_commands.describe(
        member="The member you want to unban", reason="The reason for the unban"
    )
    @app_commands.guild_only()
        ...
        async for entry in interaction.guild.bans():
            if member.lower() == entry.user.name.lower():
                ...
                embed = discord.Embed(
                    ...
                    description=f"{interaction.user.mention} unbanned {banned_user.mention}",
                    color=discord.Colour.from_rgb(213, 39, 62),
                )
                embed.add_field(name="Reason", value=reason)
                await log_channel.send(embed=embed)
                await interaction.response.send_message(
                    f"{banned_user.mention} successfully unbanned.", ephemeral=True
                )
            else:
    ...
    async def purge(
        ...
        await interaction.response.send_message(
            f"Starting to purge {interaction.channel.mention}...", ephemeral=True
        )
        ...
        embed = discord.Embed(
            ...
            description=f"{interaction.user.mention} purged {interaction.channel.mention}",
            color=discord.Colour.from_rgb(213, 39, 62),
        )
        ...
        await interaction.followup.send(
            f"{interaction.channel.mention} successfully purged.", ephemeral=True
        )
    ...
    async def timeout(
        ...
        if duration in ["60 secs", "5 mins", "10 mins", "1 hour", "1 day", "1 week"]:
            ...
            embed = discord.Embed(
                ...
                description=f"{interaction.user.mention} timed out {member.mention}",
                color=discord.Colour.from_rgb(213, 39, 62),
            )
            ...
            await log_channel.send(embed=embed)
            await interaction.response.send_message(
                f"{member.mention} successfully timed out.", ephemeral=True
            )
        else:
            await interaction.response.send_message(
    ...
    async def timeout_remove(
        ...
        if member.is_timed_out():
            ...
            embed = discord.Embed(
                title="Removed time out",
                description=f"{interaction.user.mention} removed the time out from {member.mention}",
                color=discord.Colour.from_rgb(213, 39, 62),
            )
            embed.add_field(name="Reason", value=reason)
            await log_channel.send(embed=embed)
            await interaction.response.send_message(
                f"successfully removed the time out from {member.mention}.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
